...
// the compiler
cc::cc()
    : objs(new Objs()), readModuleFileF(&defReadModuleFile),
      readModuleF(&defReadModule), readExprDefnF(&defReadExprDefn),
      readExprF(&defReadExpr), runModInlinePass(true),
      genInterpretedMatch(false), checkMatchReachability(true),
      lowerPrimMatchTables(false), unreachableMatchRowsPtr(nullptr),
      drainingDefs(false) {
  ...
  TClass *cvtc =
      new TClass("Convert", 2, cvtcms, FunDeps(), LexicalAnnotation::null());
  this->tenv->bind("Convert", UnqualifierPtr(cvtc));

  // support equality constraints
  this->tenv->bind(EqualTypes::constraintName(),
                   UnqualifierPtr(new EqualTypes()));
  this->tenv->bind(NotEqualTypes::constraintName(),
                   UnqualifierPtr(new NotEqualTypes()));

  // support destructuring types
  this->tenv->bind(DeconstructP::constraintName(),
                   UnqualifierPtr(new DeconstructP()));

  // support reversing opaque type aliases
  this->tenv->bind(DataP::constraintName(), UnqualifierPtr(new DataP()));

  // support type-level string functions (a hack until type and value levels can
  // be merged)
  ...
  this->tenv->bind(Existentials::constraintName(),
                   UnqualifierPtr(new Existentials()));

  // support intro/elim of recursive types
  this->tenv->bind(FixIsoRecur::constraintName(),
                   UnqualifierPtr(new FixIsoRecur()));
  ...
  this->tenv->bind(CtorVerifier::constraintName(),
                   UnqualifierPtr(new CtorVerifier()));

  // support deconstructing records (compile-time reflection on records)
  this->tenv->bind(RecordDeconstructor::constraintName(),
                   UnqualifierPtr(new RecordDeconstructor()));

  // support deconstructing variants (compile-time reflection on variants)
  this->tenv->bind(VariantDeconstructor::constraintName(),
                   UnqualifierPtr(new VariantDeconstructor()));

  // support appending (appendable) types
  this->tenv->bind(AppendsToUnqualifier::constraintName(),
                   UnqualifierPtr(new AppendsToUnqualifier()));

  // support connecting to remote processes
  initNetworkDefs(*this);

  // support sub-process I/O
  this->tenv->bind(ProcessP::constraintName(),
                   UnqualifierPtr(new ProcessP(fv)));

  // initialize the macro environment (maybe this should be user-controlled, the
  // set of macros is hard-coded for now)
...
SearchEntries cc::search(const MonoTypePtr &src, const MonoTypePtr &dst) {
  return hobbes::search(*this, this->searchCache, src, dst);
}
SearchEntries cc::search(const ExprPtr &e, const MonoTypePtr &dst) {
  return hobbes::search(*this, this->searchCache, e, dst);
}
SearchEntries cc::search(const std::string &e, const MonoTypePtr &dst) {
  return search(readExpr(e), dst);
}
SearchEntries cc::search(const std::string &e, const std::string &t) {
...
bool cc::hasValueBinding(const std::string &vname) {
  // either we have a bound/compiled mono-typed value, or we have a polytype
  // value (through a generated or user-defined type class)
...
//   * explicit closure construction (for lexically-scoped variables in
//   functions)
...
// need to "drain" those (ie: commit those definitions).  However, because we
// can
// qualify on recursive types, the set of residual definitions may be mutually
// recursive.
// This means that the unsweeten/drain process needs to be mutually recursive as
// well,
// and we need to accumulate the "big set" of definitions before compiling them
// in one
// big mutually-recursive set.
//
ExprPtr cc::unsweetenExpression(const TEnvPtr &te, const std::string &vname,
                                const ExprPtr &e) {
  Definitions ds;

  ExprPtr result;
  try {
    result = macroExpand(unqualifyTypes(
        te, validateType(te, vname, closureConvert(this->tenv, vname, e), &ds),
        &ds));
...
void cc::drainUnqualifyDefs(const Definitions &ds) {
  ...
  // forward declare the polymorphic functions, batch letrec compile the
  ...
  for (Definitions::const_iterator d = ds.begin(); d != ds.end(); ++d) {
    ...
    bool forwardDeclared =
        this->tenv->hasBinding(vname) && !hasValueBinding(vname);

    ExprPtr ne =
        forwardDeclared
            ? ExprPtr(new Assump(e, this->tenv->lookup(vname)->instantiate(),
                                 e->la()))
            : e;
    ...
    if (isMonotype(xety)) {
      ...
      if (forwardDeclared) {
        if (vname.substr(0, 4) == ".rfn" ||
            isMonotype(this->tenv->lookup(vname))) {
          throw annotated_error(*xe,
                                "Internal error, residual instance function '" +
                                    vname + "' should have had monotype '" +
                                    show(this->tenv->lookup(vname)) +
                                    "' but instead inferred '" + show(xety) +
                                    "' in:\n  " + showAnnotated(xe));
        }
...
void cc::define(const std::string &vname, const ExprPtr &e) {
  ...
  // define this either as a monotyped value (in which case it can be
  // machine-represented)
  //                 or as a polytyped value (in which case we need to
  //                 internalize the definition in a hidden type class)
  bool forwardDeclared = this->tenv->hasBinding(vname);

  ExprPtr ne = forwardDeclared
                   ? ExprPtr(new Assump(
                         e, this->tenv->lookup(vname)->instantiate(), e->la()))
                   : e;
  ExprPtr xe = unsweetenExpression(this->tenv, vname, ne);
  PolyTypePtr xety = hobbes::generalize(xe->type());
    ...
    if (forwardDeclared) {
      if (isMonotype(this->tenv->lookup(vname))) {
        throw annotated_error(
            *xe, "Internal error, residual instance function '" + vname +
                     "' should have had monotype '" +
                     show(this->tenv->lookup(vname)) +
                     "' but instead inferred '" + show(xety) + "'");
      }
      this->tenv->unbind(vname);
...
//  here we can just piggyback off of the existing type class /
//  instance-generator system
//  to create a private type class with one instance generator matching this
//  type signature
...
void cc::defineTypeAlias(const std::string &name, const str::seq &argNames,
...
struct repTypeAliasesF : public switchTyFn {
  ...
  MonoTypePtr with(const TApp *v) const {
      ...
      if (td != this->ttyDefs.end()) {
        if (v->args().size() != td->second.first.size()) {
          throw std::runtime_error(
              "The type constructor '" + f->name() + "' requires exactly " +
              str::from(td->second.first.size()) + " arguments");
        }
    ...
    return MonoTypePtr(
        TApp::make(switchOf(v->fn(), *this), switchOf(v->args(), *this)));
  }
  ...
  MonoTypePtr with(const FixedArray *v) const {
    return MonoTypePtr(FixedArray::make(switchOf(v->type(), *this),
                                        switchOf(v->length(), *this)));
  }
  MonoTypePtr with(const Array *v) const {
    return MonoTypePtr(Array::make(switchOf(v->type(), *this)));
  }
  MonoTypePtr with(const Variant *v) const {
    return MonoTypePtr(Variant::make(switchOf(v->members(), *this)));
  }
  MonoTypePtr with(const Record *v) const {
    return MonoTypePtr(Record::make(switchOf(v->members(), *this)));
  }
  MonoTypePtr with(const Func *v) const {
    return MonoTypePtr(Func::make(switchOf(v->argument(), *this),
                                  switchOf(v->result(), *this)));
  }
  MonoTypePtr with(const Exists *v) const {
    return MonoTypePtr(
        Exists::make(v->absTypeName(), switchOf(v->absType(), *this)));
  }
  MonoTypePtr with(const Recursive *v) const {
    return MonoTypePtr(
        Recursive::make(v->recTypeName(), switchOf(v->recType(), *this)));
  }
...
// map C++ types
PolyTypePtr cc::opaquePtrPolyType(const std::type_info &ti, unsigned int sz,
                                  bool inStruct) {
...
MonoTypePtr cc::opaquePtrMonoType(const std::type_info &ti, unsigned int sz,
                                  bool inStruct) {
  // we don't necesarily *HAVE* to make this type opaque, if we've previously
  // been given a type mapping AND the type has a pointer representation
  ...
  if (t != this->typeAliases.end() && hasPointerRep(t->second)) {
    ...
    // OK, we don't know what this type looks like so we'll give it an opaque
    // pointer type
...
void cc::overload(const std::string &tyclass, const MonoTypes &tys) {
  ...
  if (c.get() == 0) {
    throw std::runtime_error("Cannot define overload in '" + tyclass +
                             "', class does not exist.");
  } else if (c->members().size() != 0) {
    throw std::runtime_error("Cannot define partial overload for type class '" +
                             tyclass + "', which has " +
                             str::from(c->members().size()) + " members.");
  }
  ...
  if (tgenSize(tys) == 0) {
    Definitions ds;
    c->insert(typeEnv(), TCInstancePtr(new TCInstance(
                             tyclass, tys, insts, LexicalAnnotation::null())),
              &ds);
    drainUnqualifyDefs(ds);
  } else {
    throw std::runtime_error(
        "cc::overload forgot how to produce instance functions");
  }
}
...
void cc::overload(const std::string &tyclass, const MonoTypes &tys,
  ...
  if (tgenSize(tys) == 0) {
    Definitions ds;
    c->insert(typeEnv(),
              TCInstancePtr(new TCInstance(tyclass, tys, insts, e->la())), &ds);
    drainUnqualifyDefs(ds);
  } else {
    throw std::runtime_error(
        "cc::overload forgot how to produce instance functions");
...
MonoTypePtr cc::defineNamedType(const std::string &name,
...
llvm::IRBuilder<> *cc::builder() const { return this->jit.builder(); }

llvm::Module *cc::module() const {
  return const_cast<cc *>(this)->jit.module();
}

void cc::dumpTypeEnv() const { std::cout << showTypeEnv() << std::endl; }

void cc::dumpTypeEnv(str::seq *syms, str::seq *types) const {
  const TEnv::PolyTypeEnv &ptenv = this->tenv->typeEnvTable();

  for (TEnv::PolyTypeEnv::const_iterator te = ptenv.begin(); te != ptenv.end();
       ++te) {
...
const TEnvPtr &cc::typeEnv() const { return this->tenv; }

void cc::dumpModule() { this->jit.dump(); }
...
void cc::bindExternFunction(const std::string &fname, const MonoTypePtr &fty,
                            void *fn) {
  this->tenv->bind(fname, generalize(fty));
  this->jit.bindGlobal(fname, fty, fn);
}

void *cc::memalloc(size_t sz) { return this->jit.memalloc(sz); }

inline TEnvPtr allocTEnvFrame(const str::seq &names, const MonoTypes &tys,
                              const TEnvPtr &ptenv) {
  if (names.size() == 0) {
    return ptenv;
  } else {
...
void *cc::unsafeCompileFn(const MonoTypePtr &retTy, const str::seq &tnames,
  ...
  if (names.size() == 0 && argTys.size() == 1 && isUnit(argTys[0])) {
    ...
    ss << "Function parameter name list [" << str::cdelim(names, ", ")
       << "] and parameter type list [" << str::cdelim(show(argTys), ", ")
       << "] have inconsistent lengths.";
    throw annotated_error(*exp, ss.str());
  }

  return this->jit.reifyMachineCodeForFn(
      retTy, names, argTys,
      unsweetenExpression(allocTEnvFrame(names, argTys, this->tenv),
                          assume(exp, retTy, exp->la())));
}

void *cc::unsafeCompileFn(const MonoTypePtr &fnTy, const str::seq &names,
                          const ExprPtr &exp) {
...
void cc::releaseMachineCode(void *f) { this->jit.releaseMachineCode(f); }
...
bool cc::requireMatchReachability() const {
  return this->checkMatchReachability;
}

void cc::alwaysLowerPrimMatchTables(bool f) { this->lowerPrimMatchTables = f; }
bool cc::alwaysLowerPrimMatchTables() const {
  return this->lowerPrimMatchTables;
}
}
