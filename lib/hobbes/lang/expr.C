...
Expr::Expr(int cid, const LexicalAnnotation &la)
    : LexicallyAnnotated(la), cid(cid) {}
Expr::~Expr() {}
...
bool PrimPtrLT::operator()(const PrimitivePtr &p0,
                           const PrimitivePtr &p1) const {
  return *p0 < *p1;
}
...
void Unit::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}
...
void Bool::show(std::ostream &out) const {
  out << (this->x ? "true" : "false");
}
void Bool::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}
...
void Char::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}
...
void Byte::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}
...
void Short::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}
...
void Int::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}
...
void Long::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}
...
void Float::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}
...
void Double::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}
bool Double::equiv(const Double &rhs) const { return this->x == rhs.x; }
bool Double::lt(const Double &rhs) const { return this->x < rhs.x; }
MonoTypePtr Double::primType() const {
  return MonoTypePtr(Prim::make("double"));
}

Var::Var(const std::string &id, const LexicalAnnotation &la)
    : Base(la), id(id) {}
...
void Var::showAnnotated(std::ostream &out) const {
  show(out);
  showTy(out, type());
}

Let::Let(const std::string &id, const ExprPtr &e, const ExprPtr &b,
         const LexicalAnnotation &la)
    : Base(la), id(id), e(e), b(b) {}
bool Let::operator==(const Let &rhs) const {
  return this->id == rhs.id && *this->e == *rhs.e && *this->b == *rhs.b;
}
...
Expr *Let::clone() const {
  return new Let(this->id, ExprPtr(this->e->clone()), ExprPtr(this->b->clone()),
                 la());
}
...
LetRec::LetRec(const Bindings &bs, const ExprPtr &e,
               const LexicalAnnotation &la)
    : Base(la), bs(bs), e(e) {}
...
bool LetRec::operator==(const LetRec &rhs) const {
  if (this->bs.size() != rhs.bs.size() || !(*this->e == *rhs.e)) {
    ...
    for (size_t i = 0; i < this->bs.size(); ++i) {
      if ((this->bs[i].first != rhs.bs[i].first) ||
          !(*this->bs[i].second == *rhs.bs[i].second)) {
        return false;
      }
    }
    return true;
...
Fn::Fn(const VarNames &vs, const ExprPtr &e, const LexicalAnnotation &la)
    : Base(la), vs(vs), e(e) {
  // to make type-checking homogeneous, we equate the empty parameter list with
  // a parameter list containing just unit
  //   (unit is compiled away to nothing, so it's equivalent)
  if (this->vs.size() == 0) {
    this->vs.push_back("_");
  }
}
bool Fn::operator==(const Fn &rhs) const {
  return this->vs == rhs.vs && *this->e == *rhs.e;
}
...
Expr *Fn::clone() const {
  return new Fn(this->vs, ExprPtr(this->e->clone()), la());
}
...
App::App(const ExprPtr &fne, const Exprs &argl, const LexicalAnnotation &la)
    : Base(la), fne(fne), argl(argl) {
  // to make type-checking homogeneous, we equate the empty parameter list with
  // a parameter list containing just unit
  //   (unit is compiled away to nothing, so it's equivalent)
  if (this->argl.size() == 0) {
...
Expr *App::clone() const {
  return new App(ExprPtr(this->fne->clone()), clones(this->argl), la());
}
...
Assign::Assign(const ExprPtr &lhs, const ExprPtr &rhs,
               const LexicalAnnotation &la)
    : Base(la), lhs(lhs), rhs(rhs) {}
...
Expr *Assign::clone() const {
  return new Assign(ExprPtr(this->lhs->clone()), ExprPtr(this->rhs->clone()),
                    la());
}
...
MkArray::MkArray(const Exprs &es, const LexicalAnnotation &la)
    : Base(la), es(es) {}
...
MkVariant::MkVariant(const std::string &lbl, const ExprPtr &e,
                     const LexicalAnnotation &la)
    : Base(la), lbl(lbl), e(e) {}
bool MkVariant::operator==(const MkVariant &rhs) const {
  return this->lbl == rhs.lbl && *this->e == *rhs.e;
}
...
Expr *MkVariant::clone() const {
  return new MkVariant(this->lbl, ExprPtr(this->e->clone()), la());
}
...
MkRecord::MkRecord(const FieldDefs &fs, const LexicalAnnotation &la)
    : Base(la), fs(fs) {}
bool MkRecord::operator==(const MkRecord &rhs) const {
  if (this->fs.size() != rhs.fs.size()) {
    ...
    for (size_t i = 0; i < this->fs.size(); ++i) {
      if ((this->fs[i].first != rhs.fs[i].first) ||
          !(*this->fs[i].second == *rhs.fs[i].second)) {
        return false;
      }
    }
    return true;
...
bool MkRecord::isTuple() const {
  return this->fs.size() > 0 && this->fs[0].first.size() > 0 &&
         this->fs[0].first[0] == '.';
}
Expr *MkRecord::clone() const {
  FieldDefs cfs;
  for (FieldDefs::const_iterator f = this->fs.begin(); f != this->fs.end();
       ++f) {
...
AIndex::AIndex(const ExprPtr &arr, const ExprPtr &i,
               const LexicalAnnotation &la)
    : Base(la), arr(arr), i(i) {}
bool AIndex::operator==(const AIndex &rhs) const {
  return *this->arr == *rhs.arr && *this->i == *rhs.i;
}
...
Expr *AIndex::clone() const {
  return new AIndex(ExprPtr(this->arr->clone()), ExprPtr(this->i->clone()),
                    la());
}
...
Case::Case(const ExprPtr &v, const Bindings &bs, const ExprPtr &def,
           const LexicalAnnotation &la)
    : Base(la), v(v), bs(bs), def(def) {}

Case::Case(const ExprPtr &v, const Bindings &bs, const LexicalAnnotation &la)
    : Case(v, bs, ExprPtr(), la) {}
...
bool Case::operator==(const Case &rhs) const {
  if ((this->bs.size() != rhs.bs.size()) || !(*this->v == *rhs.v)) {
    ...
    for (size_t i = 0; i < this->bs.size(); ++i) {
      if ((this->bs[i].selector != rhs.bs[i].selector) ||
...
bool Case::hasBinding(const std::string &bn) const {
  for (Bindings::const_iterator b = this->bs.begin(); b != this->bs.end();
       ++b) {
    if (b->selector == bn) {
      return true;
    }
...
void Case::addBinding(const std::string &selector, const std::string &vname,
                      const ExprPtr &exp) {
...
Expr *Case::clone() const {
  ...
  return new Case(ExprPtr(this->v->clone()), cbs,
                  this->def.get() ? ExprPtr(this->def->clone()) : ExprPtr(),
                  la());
}
...
Switch::Switch(const ExprPtr &v, const Bindings &bs, const ExprPtr &def,
               const LexicalAnnotation &la)
    : Base(la), v(v), bs(bs), def(def) {
...
Switch::Switch(const ExprPtr &v, const Bindings &bs,
               const LexicalAnnotation &la)
    : Switch(v, bs, ExprPtr(), la) {}
...
bool Switch::operator==(const Switch &rhs) const {
  if (this->bs.size() != rhs.bs.size() || !(*this->v == *rhs.v)) {
    ...
    for (size_t i = 0; i < this->bs.size(); ++i) {
      if (!(*this->bs[i].value == *rhs.bs[i].value) ||
          !(*this->bs[i].exp == *rhs.bs[i].exp)) {
        return false;
      }
    }
...
Expr *Switch::clone() const {
  ...
  Bindings cbs;
  for (auto b : this->bs) {
    cbs.push_back(Binding(PrimitivePtr((Primitive *)b.value->clone()),
                          ExprPtr(b.exp->clone())));
  }
...
Proj::Proj(const ExprPtr &r, const std::string &fn, const LexicalAnnotation &la)
    : Base(la), r(r), fn(fn) {}
bool Proj::operator==(const Proj &rhs) const {
  return *this->r == *rhs.r && this->fn == rhs.fn;
}
...
Expr *Proj::clone() const {
  return new Proj(ExprPtr(this->r->clone()), this->fn, la());
}
...
Assump::Assump(const ExprPtr &e, const QualTypePtr &t,
               const LexicalAnnotation &la)
    : Base(la), e(e), t(t) {}
bool Assump::operator==(const Assump &rhs) const {
  return *this->e == *rhs.e && *this->t == *rhs.t;
}
Expr *Assump::clone() const {
  return new Assump(ExprPtr(this->e->clone()), hobbes::cloneP(this->t), la());
}
...
Unpack::Unpack(const std::string &vn, const ExprPtr &pkg, const ExprPtr &body,
               const LexicalAnnotation &la)
    : Base(la), vn(vn), pkg(pkg), body(body) {}
bool Unpack::operator==(const Unpack &rhs) const {
  return this->vn == rhs.vn && *this->pkg == *rhs.pkg &&
         *this->body == *rhs.body;
}
Expr *Unpack::clone() const {
  return new Unpack(this->vn, ExprPtr(this->pkg->clone()),
                    ExprPtr(this->body->clone()), la());
}
...
// a convenience method for showing expressions with explicit type annotations
// (useful for debugging/testing the compiler)
...
std::string showAnnotated(const Expr *e) { return showAnnotated(*e); }

std::string showAnnotated(const ExprPtr &e) { return showAnnotated(*e); }
...
struct substVarF : public switchExprC<ExprPtr> {
  ...
  ExprPtr withoutNames(const str::set &ns, const ExprPtr &e) const {
    return switchOf(e, substVarF(drop(this->vm, ns), this->mapped));
  }
  ExprPtr withoutNames(const str::seq &ns, const ExprPtr &e) const {
    return withoutNames(toSet(ns), e);
  }
  ExprPtr withoutNames(const std::string &n, const ExprPtr &e) const {
    return withoutNames(toSet(list(n)), e);
  }
  ...
  ExprPtr with(const Let *v) const {
    return ExprPtr(new Let(v->var(), switchOf(v->varExpr(), *this),
                           withoutNames(v->var(), v->bodyExpr()), v->la()));
  }
  ...
  ExprPtr with(const Fn *v) const {
    return ExprPtr(
        new Fn(v->varNames(), withoutNames(v->varNames(), v->body()), v->la()));
  }
  ...
  ExprPtr with(const AIndex *v) const {
    return ExprPtr(new AIndex(switchOf(v->array(), *this),
                              switchOf(v->index(), *this), v->la()));
  }
  ...
  ExprPtr with(const Case *v) const {
    ...
    for (Case::Bindings::const_iterator cb = cbs.begin(); cb != cbs.end();
         ++cb) {
      rcbs.push_back(Case::Binding(cb->selector, cb->vname,
                                   withoutNames(cb->vname, cb->exp)));
    }
  ...
  ExprPtr with(const Unpack *v) const {
    return ExprPtr(new Unpack(v->varName(), switchOf(v->package(), *this),
                              withoutNames(v->varName(), v->expr()), v->la()));
  }
};
...
bool isConst(const ExprPtr &e) { return switchOf(e, isConstP()); }

// a convenient encapsulation of type-directed term transformation (e.g.: for
// unqualification)
QualTypePtr switchExprTyFn::withTy(const QualTypePtr &qt) const { return qt; }
...
Case::Bindings switchOf(const Case::Bindings &bs,
...
Switch::Bindings switchOf(const Switch::Bindings &bs,
                          const switchExpr<ExprPtr> &f) {
  Switch::Bindings r;
  for (auto b : bs) {
...
ExprPtr switchExprTyFn::withConst(const Expr *v) const {
  return wrapWithTy(v->type(), v->clone());
}
ExprPtr switchExprTyFn::with(const Var *v) const {
  return wrapWithTy(v->type(), v->clone());
}
ExprPtr switchExprTyFn::with(const Let *v) const {
  return wrapWithTy(v->type(),
                    new Let(v->var(), switchOf(v->varExpr(), *this),
                            switchOf(v->bodyExpr(), *this), v->la()));
}
ExprPtr switchExprTyFn::with(const Fn *v) const {
  return wrapWithTy(v->type(),
                    new Fn(v->varNames(), switchOf(v->body(), *this), v->la()));
}
ExprPtr switchExprTyFn::with(const App *v) const {
  return wrapWithTy(v->type(), new App(switchOf(v->fn(), *this),
                                       switchOf(v->args(), *this), v->la()));
}
ExprPtr switchExprTyFn::with(const Assign *v) const {
  return wrapWithTy(v->type(),
                    new Assign(switchOf(v->left(), *this),
                               switchOf(v->right(), *this), v->la()));
}
ExprPtr switchExprTyFn::with(const MkArray *v) const {
  return wrapWithTy(v->type(),
                    new MkArray(switchOf(v->values(), *this), v->la()));
}
ExprPtr switchExprTyFn::with(const MkVariant *v) const {
  return wrapWithTy(
      v->type(),
      new MkVariant(v->label(), switchOf(v->value(), *this), v->la()));
}
ExprPtr switchExprTyFn::with(const MkRecord *v) const {
  return wrapWithTy(v->type(),
                    new MkRecord(switchOf(v->fields(), *this), v->la()));
}
ExprPtr switchExprTyFn::with(const AIndex *v) const {
  return wrapWithTy(v->type(),
                    new AIndex(switchOf(v->array(), *this),
                               switchOf(v->index(), *this), v->la()));
}
ExprPtr switchExprTyFn::with(const Proj *v) const {
  return wrapWithTy(
      v->type(), new Proj(switchOf(v->record(), *this), v->field(), v->la()));
}
ExprPtr switchExprTyFn::with(const Assump *v) const {
  return wrapWithTy(v->type(), new Assump(switchOf(v->expr(), *this),
                                          withTy(v->ty()), v->la()));
}
ExprPtr switchExprTyFn::with(const Pack *v) const {
  return wrapWithTy(v->type(), new Pack(switchOf(v->expr(), *this), v->la()));
}
ExprPtr switchExprTyFn::with(const Unpack *v) const {
  return wrapWithTy(v->type(),
                    new Unpack(v->varName(), switchOf(v->package(), *this),
...
ExprPtr switchExprTyFn::with(const LetRec *v) const {
  ...
  return wrapWithTy(v->type(),
                    new LetRec(bs, switchOf(v->bodyExpr(), *this), v->la()));
}
...
ExprPtr switchExprTyFn::with(const Case *v) const {
  ...
  return wrapWithTy(v->type(),
                    new Case(switchOf(v->variant(), *this),
                             switchOf(v->bindings(), *this), de, v->la()));
}
...
ExprPtr switchExprTyFn::with(const Switch *v) const {
  ...
  return wrapWithTy(v->type(),
                    new Switch(switchOf(v->expr(), *this),
                               switchOf(v->bindings(), *this), de, v->la()));
}
...
QualTypePtr switchExprTyFnM::withTy(const QualTypePtr &qt) const { return qt; }
...
// lift any inferred type information to explicit assumptions (avoid losing type
// info over multiple runs of inference)
struct liftTypeAsAssumpF : public switchExprTyFn {
  ...
  ExprPtr wrapWithTy(const QualTypePtr &qty, Expr *e) const {
    if (qty != QualTypePtr()) {
      return setTy(ExprPtr(new Assump(setTy(ExprPtr(e), qty), qty, e->la())),
                   qty);
    } else {
      return ExprPtr(e);
    }
...
const MonoTypePtr &requireMonotype(const ExprPtr &e) {
  if (e->type() == QualTypePtr()) {
    throw annotated_error(
        *e, "Expression '" + show(e) +
                "' not explicitly annotated.  Internal compiler error.");
  }

  if (e->type()->constraints().size() > 0) {
    Constraints cs = simplifyVarNames(e->type())->constraints();
    std::ostringstream ss;
    ss << "Failed to compile expression due to unresolved type constraint"
       << (cs.size() > 1 ? "s" : "") << ":";
    for (const auto &c : cs) {
      ss << "\n  " << show(c);
    }
...
struct stripAssumpF : public switchExprTyFn {
  ExprPtr with(const Assump *v) const { return switchOf(v->expr(), *this); }
};
...
struct freeVarF : public switchExprC<VarSet> {
  ...
  VarSet with(const Let *v) const {
    return setUnion(
        list(freeVars(v->varExpr()),
             setDifference(freeVars(v->bodyExpr()), toSet(list(v->var())))));
  }
  VarSet with(const Fn *v) const {
    return setDifference(freeVars(v->body()), toSet(v->varNames()));
  }
  VarSet with(const App *v) const {
    return setUnion(cons(freeVars(v->fn()), switchOf(v->args(), *this)));
  }
  VarSet with(const Assign *v) const {
    return setUnion(list(freeVars(v->left()), freeVars(v->right())));
  }
  VarSet with(const MkArray *v) const {
    return setUnion(switchOf(v->values(), *this));
  }
  VarSet with(const MkVariant *v) const { return freeVars(v->value()); }
  VarSet with(const MkRecord *v) const {
    return setUnion(switchOf(exprs(v->fields()), *this));
  }
  VarSet with(const AIndex *v) const {
    return setUnion(list(freeVars(v->array()), freeVars(v->index())));
  }
  ...
  VarSet with(const Unpack *v) const {
    return setUnion(
        list(freeVars(v->package()),
             setDifference(freeVars(v->expr()), toSet(list(v->varName())))));
  }
  ...
  VarSet with(const Case *v) const {
    ...
    for (Case::Bindings::const_iterator b = v->bindings().begin();
         b != v->bindings().end(); ++b) {
...
VarSet freeVars(const ExprPtr &e) { return switchOf(e, freeVarF()); }

VarSet freeVars(const Expr &e) { return switchOf(e, freeVarF()); }
...
struct etvarNamesF : public switchExprC<UnitV> {
  ...
  UnitV an(const Expr *v) const {
    if (v->type()) {
      tvarNames(v->type(), this->out);
    }
    return unitv;
  }
...
ComprehensionDef::ComprehensionDef(const std::string &varname,
                                   const ExprPtr &ex)
    : isfilter(false), varname(varname), ex(ex) {}

ComprehensionDef::ComprehensionDef(const ExprPtr &ex)
    : isfilter(true), ex(ex) {}

ExprPtr desugarCompFilter(const ExprPtr &ex, const ExprPtr &filterE,
                          const LexicalAnnotation &la) {
  return fncall(var("if", la), list(filterE, ExprPtr(new MkArray(list(ex), la)),
                                    ExprPtr(new MkArray(Exprs(), la))),
                la);
}

ExprPtr desugarCompMap(const std::string &vn, const ExprPtr &ex,
                       const ExprPtr &ae, const LexicalAnnotation &la) {
...
ExprPtr desugarComprehensionFrom(const ExprPtr &ex,
                                 const ComprehensionDefs &cdefs, unsigned int i,
                                 const LexicalAnnotation &la) {
  i = i - 1; // translate from count to index

  const ComprehensionDef &cdef = cdefs[i];
  ExprPtr nextEx = cdef.isfilter
                       ? desugarCompFilter(ex, cdef.ex, la)
                       : desugarCompMap(cdef.varname, ex, cdef.ex, la);

  if (i == 0) {
    return nextEx;
  } else {
    return desugarCompConcat(desugarComprehensionFrom(nextEx, cdefs, i, la),
                             la);
  }
}

ExprPtr desugarComprehension(const ExprPtr &ex, const ComprehensionDefs &cdefs,
                             const LexicalAnnotation &la) {
...
ExprPtr fmtFoldConst(const ExprPtr &e, const std::string &c) {
  return fncall(var("append", e->la()), list(e, ExprPtr(mkarray(c, e->la()))),
                e->la());
}

ExprPtr fmtFoldExpr(const ExprPtr &e, const std::string &expr) {
  return fncall(var("append", e->la()),
                list(e, fncall(var("format", e->la()), list(var(expr, e->la())),
                               e->la())),
                e->la());
}
...
ExprPtr mkFormatExpr(const std::string &fmt, const LexicalAnnotation &la) {
  if (fmt.size() == 0) {
    ...
    return str::foldWithFormat(str::trimq(fmt, '`'), ExprPtr(mkarray("", la)),
                               &fmtFoldConst, &fmtFoldExpr);
  }
}
...
ExprPtr mkTimespanExpr(const str::seq &x, const LexicalAnnotation &la) {
  return assume(fncall(var("convert", la),
                       list(ExprPtr(new Long(readTimespan(x), la))), la),
                qualtype(primty("timespan", primty("long"))), la);
}
...
ExprPtr mkTimeExpr(const std::string &x, const LexicalAnnotation &la) {
  return assume(
      fncall(var("convert", la), list(ExprPtr(new Long(readTime(x), la))), la),
      qualtype(primty("time", primty("long"))), la);
}
...
ExprPtr mkDateTimeExpr(const std::string &x, const LexicalAnnotation &la) {
  return assume(fncall(var("convert", la),
                       list(ExprPtr(new Long(readDateTime(x), la))), la),
                qualtype(primty("datetime", primty("long"))), la);
}
...
void decode(PrimitivePtr *p, std::istream &in) { decode((ExprPtr *)p, in); }
...
void encode(const ExprPtr &e, std::ostream &out) {
  try {
    ...
    std::cout << "while encoding expression:" << std::endl
              << showAnnotated(e) << std::endl;
    throw;
  }
}
...
void decode(ExprPtr *out, std::istream &in) {
  ...
  default:
    throw std::runtime_error(
        "Internal error, cannot switch on unknown expression type: " +
        str::from(cid));
  }
...
// determine whether or not an expression is fully annotated everywhere with a
// singular type
struct hasSingularTypeF : public switchExprC<bool> {
  bool withConst(const Expr *v) const { return d(v); }

  bool with(const Var *v) const { return d(v); }
  ...
  bool with(const Fn *v) const { return d(v) && r(v->body()); }
  ...
  bool with(const MkVariant *v) const { return d(v) && r(v->value()); }
  ...
  bool with(const Case *v) const {
    if (!d(v) || !r(v->variant()) || (v->defaultExpr() && !r(v->defaultExpr())))
      return false;
    for (auto b : v->bindings()) {
      if (!r(b.exp))
        return false;
  ...
  bool with(const Switch *v) const {
    if (!d(v) || !r(v->expr()) || (v->defaultExpr() && !r(v->defaultExpr())))
      return false;
    for (auto b : v->bindings()) {
      if (!r(b.exp))
        return false;
  ...
  bool with(const Proj *v) const { return d(v) && r(v->record()); }

  bool with(const Assump *v) const {
    return d(v) && r(v->expr()) && isMonoSingular(v->ty());
  }

  bool with(const Pack *v) const { return d(v) && r(v->expr()); }
...
private:
  bool d(const Expr *v) const {
    return v->type().get() && isMonoSingular(v->type());
  }
...
struct tgenSizeExprF : public switchExprC<int> {
  int withConst(const Expr *v) const { return d(v); }

  int with(const Var *v) const { return d(v); }
  ...
  int with(const Fn *v) const { return c(d(v), r(v->body())); }
  ...
  int with(const MkVariant *v) const { return c(d(v), r(v->value())); }
  ...
  int with(const Proj *v) const { return c(d(v), r(v->record())); }

  int with(const Assump *v) const {
    return c(c(d(v), r(v->expr())), tgenSize(v->ty()));
  }

  int with(const Pack *v) const { return c(d(v), r(v->expr())); }
