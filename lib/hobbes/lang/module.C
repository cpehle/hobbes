...
/* modules */
Module::Module(const std::string &mname, const ModuleDefs &defs)
    : mname(mname), defs(defs) {}
...
void Module::show(std::ostream &out) const {
  ...
  for (ModuleDefs::const_iterator md = this->defs.begin();
       md != this->defs.end(); ++md) {
    (*md)->show(out);
    out << std::endl;
  }
...
/* module defs */
ModuleDef::ModuleDef(int cid, const LexicalAnnotation &la)
    : LexicallyAnnotated(la), cid(cid) {}
int ModuleDef::case_id() const { return this->cid; }
ModuleDef::~ModuleDef() {}

// module imports
MImport::MImport(const std::string &p, const std::string &n,
...
// type definitions
MTypeDef::MTypeDef(Visibility v, const std::string &tname,
                   const str::seq &targs, const MonoTypePtr &t,
                   const LexicalAnnotation &la)
    : Base(la), v(v), tname(tname), targs(targs), t(t) {}
...
void MTypeDef::show(std::ostream &out) const {
  if (this->v == Transparent) {
    out << "type " << this->tname << (this->targs.size() > 0 ? " " : "")
        << str::cdelim(this->targs, " ") << " = " << hobbes::show(this->t);
  } else {
    out << "data " << this->tname << (this->targs.size() > 0 ? " " : "")
        << str::cdelim(this->targs, " ") << " = " << hobbes::show(this->t);
  }
}

// variable type bindings
MVarTypeDef::MVarTypeDef(const std::string &vname, const QualTypePtr &qty,
                         const LexicalAnnotation &la)
    : Base(la), vname(vname), qty(qty) {}
...
MVarDef::MVarDef(const str::seq &vargl, const ExprPtr &e,
                 const LexicalAnnotation &la)
    : Base(la), vargl(nameBlanks(vargl)), expr(e) {}
...
// class declarations
ClassDef::ClassDef(const Constraints &cs, const std::string &cname,
                   const str::seq &tvars, const CFunDepDefs &fdeps,
                   const MVarTypeDefs &mvtydefs, const LexicalAnnotation &la)
    : Base(la), cs(cs), cname(cname), tvars(tvars), fdeps(fdeps),
      mvtydefs(mvtydefs) {}
...
void ClassDef::show(std::ostream &out) const {
  ...
  out << " where" << std::endl;
  for (MVarTypeDefs::const_iterator mvtd = this->mvtydefs.begin();
       mvtd != this->mvtydefs.end(); ++mvtd) {
    out << "  ";
    (*mvtd)->show(out);
    out << std::endl;
  }
...
InstanceDef::InstanceDef(const Constraints &cs, const std::string &cname,
                         const MonoTypes &targs, const MVarDefs &mdefs,
                         const LexicalAnnotation &la)
...
void InstanceDef::show(std::ostream &out) const {
  out << "instance (" << str::cdelim(hobbes::show(this->cs), ", ") << ") => "
      << this->cname << " " << str::cdelim(hobbes::show(this->targs), " ")
      << " where" << std::endl;
  for (MVarDefs::const_iterator mvd = this->mdefs.begin();
       mvd != this->mdefs.end(); ++mvd) {
    out << "  ";
    (*mvd)->show(out);
    out << std::endl;
  }
...
std::string show(const ClassDef *cd) { return show(*cd); }
...
std::string show(const InstanceDef *id) { return show(*id); }
...
std::string show(const CFunDepDef &fundep) {
  return str::cdelim(fundep.first, " ") + " -> " +
         str::cdelim(fundep.second, " ");
}
...
MVarDefs substitute(const MonoTypeSubst &s, const MVarDefs &vds) {
  ...
  for (MVarDefs::const_iterator vd = vds.begin(); vd != vds.end(); ++vd) {
    result.push_back(MVarDefPtr(new MVarDef(
        (*vd)->varWithArgs(), substitute(s, (*vd)->varExpr()), (*vd)->la())));
  }
  return result;
}
}
